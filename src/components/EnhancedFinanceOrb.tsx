...
interface EnhancedFinanceOrbProps {
  isActive?: boolean;
  image?: string;
}

export default function EnhancedFinanceOrb({ isActive = false, image }: EnhancedFinanceOrbProps) {
  return (
    <div className="relative flex items-center justify-center w-72 h-72 mx-auto">
      ...
      <motion.div
        ...
        style={{
          background: 'conic-gradient(from 0deg, #f59e0b, #eab308, #84cc16, #f59e0b)',
          filter: 'blur(3px)',
        }}
      />
      ...
      <motion.div
        ...
        className="absolute inset-4 rounded-full bg-gradient-to-br from-yellow-300/40 via-amber-400/30 to-lime-400/40 blur-xl"
      />
      ...
      <motion.div
        ...
        className="relative z-10 w-52 h-52 rounded-full bg-gradient-to-br from-yellow-400 via-amber-500 to-orange-500 shadow-2xl overflow-hidden border-2 border-white/20"
        style={{
          boxShadow: '0 0 60px rgba(245, 158, 11, 0.6), inset 0 0 60px rgba(255, 255, 255, 0.1)',
        }}
      >
        {/* Premium Inner Gradient Layer */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/30 via-transparent to-yellow-600/20" />
        ...
        <motion.div
          ...
          style={{
            boxShadow: 'inset 0 0 30px rgba(255, 255, 255, 0.2), 0 0 20px rgba(245, 158, 11, 0.3)',
          }}
        >
          {image ? (
            <motion.img
              src={image}
              alt="Finance Intelligence"
            ...
            >
              <DollarSign className="w-20 h-20 text-white drop-shadow-lg" />
            </motion.div>
          )}
        </motion.div>
